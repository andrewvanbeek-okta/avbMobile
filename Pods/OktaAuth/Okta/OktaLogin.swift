...
public struct Login {
    ...
    public func samlStart(withPListConfig plistName: String?, view: UIViewController,
                      callback: @escaping (OktaTokenManager?, OktaError?) -> Void) {
        ...
        if !self.passwordFlow {
            // Get client configuration from Okta.plist
            if let config = Utils().getPlistConfiguration(forResourceName: plistName!) {
                OktaAuthorization().samltoBox(config, view: view) { response, error in callback(response, error) }
            }
        }
    }
    ...
    public func samlStart(_ view: UIViewController, callback: @escaping (OktaTokenManager?, OktaError?) -> Void) {
        self.samlStart(withPListConfig: "Okta", view: view) { result, error in callback(result, error) }
    }
}
